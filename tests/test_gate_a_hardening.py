...
settlement table is the oracle: it is a different SQLite file from UNBLOCK's
ledger, so an UNBLOCK crash cannot take the evidence down with it.
"""
...
def test_restart_with_fresh_connections_ask_approve_resume(tmp_path):
    ...
    ledger1 = Ledger(db)
    unblock1 = Unblock(ledger1, POLICY, FileRail(rail_db))
    assert unblock1.run_job("job-big", BIG, "renew domain") == "WAITING_APPROVAL"
    ledger1.close()  # process 1 is gone; nothing survives but the files

    ledger2 = Ledger(db)
    unblock2 = Unblock(ledger2, POLICY, FileRail(rail_db))
    assert unblock2.ledger.job("job-big")["state"] == "WAITING_APPROVAL"
    assert unblock2.resume("job-big") == "WAITING_APPROVAL"  # no decision yet
    ...
    unblock3 = Unblock(ledger3, POLICY, rail3)
    assert unblock3.resume("job-big") == "DONE"
    assert rail3.settle_count(BIG) == 1
    assert unblock3.ledger.receipt(BIG)["tx"].startswith("file-")
...
class CrashAfterSettleRail(FileRail):
    ...
    UNBLOCK can record the receipt - the worst-case crash window."""
...
def _crash_worker(db: str, rail_db: str):
    unblock = Unblock(Ledger(db), POLICY, CrashAfterSettleRail(rail_db))
    unblock.run_job("job-crash", SMALL, "fetch premium data")  # never returns
...
def test_crash_after_settle_reconciles_without_repay(tmp_path):
    ...
    assert unblock.run_job("job-retry", SMALL, "retry after crash") == "WAITING_APPROVAL"
    assert rail.settle_count(SMALL) == 1

    # Reconcile adopts the rail's settlement record: PAYING -> PAID, no new payment.
    assert unblock.reconcile("job-crash") == "DONE"
    assert rail.settle_count(SMALL) == 1
    ...
    assert unblock.resume("job-retry") == "DONE"
    assert rail.settle_count(SMALL) == 1
...
def _race_worker(db: str, rail_db: str, job_id: str, results):
    unblock = Unblock(Ledger(db), POLICY, FileRail(rail_db))
    results.append(unblock.run_job(job_id, SMALL, "race"))
...
def test_approval_for_other_terms_authorizes_nothing(tmp_path):
    ...
    assert unblock.approve(altered, actor="akiyuki") is True

    # The ledger's pinned terms don't match the approved digest: still parked.
    assert unblock.resume("job-big") == "WAITING_APPROVAL"
    ...
    assert unblock.approve(BIG, actor="akiyuki") is False
    assert unblock.resume("job-big") == "WAITING_APPROVAL"
    assert rail.settle_count(BIG) == 0
...
def test_decision_is_terminal_approve_then_reject(tmp_path):
    rail = FileRail(tmp_path / "rail.db")
    unblock = Unblock(Ledger(tmp_path / "ledger.db"), POLICY, rail)
    assert unblock.run_job("job-big", BIG, "work") == "WAITING_APPROVAL"

    assert unblock.approve(BIG, actor="akiyuki") is True
    assert unblock.reject(BIG, actor="akiyuki") is False  # cannot flip a terminal decision
    assert unblock.ledger.decision(BIG)[0] == "APPROVED"
    assert unblock.resume("job-big") == "DONE"
...
def test_decision_is_terminal_reject_then_approve(tmp_path):
    ...
    assert unblock.reject(BIG, actor="akiyuki", note="too expensive") is True
    assert unblock.approve(BIG, actor="akiyuki") is False  # cannot flip a terminal decision
    assert unblock.resume("job-big") == "FAILED"
    assert unblock.ledger.state(BIG) == "DENIED"
    assert rail.settle_count(BIG) == 0
...
class UncertainThenLookupRail(FileRail):
    ...
    def pay(self, invoice):
        from unblock.rails import SettlementUncertain
...
def test_settlement_uncertain_stays_paying_then_reconciles(tmp_path):
    ...
    unblock = Unblock(Ledger(tmp_path / "ledger.db"), POLICY, rail)
    assert unblock.run_job("job-u", SMALL, "work") == "WAITING_APPROVAL"
    assert unblock.ledger.state(SMALL) == "PAYING"  # not ASK_PENDING: no retry path
    assert rail.settle_count(SMALL) == 1

    # A rerun must not pay again.
    assert unblock.run_job("job-u2", SMALL, "retry") == "WAITING_APPROVAL"
    assert rail.settle_count(SMALL) == 1

    # resume() on a PAYING row routes through reconcile: adopts the settlement.
    assert unblock.resume("job-u") == "DONE"
    assert unblock.ledger.state(SMALL) == "PAID"
    assert rail.settle_count(SMALL) == 1
